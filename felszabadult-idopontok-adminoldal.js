...
const hozzaadGomb =
  document.getElementById("hozzaad");
...
const kivalasztottLista =
  document.getElementById("kivalasztottLista");
...
function kirajzolKivalasztottIdopontok() {
  ...
  if (kivalasztottIdopontok.length === 0) {
    kivalasztottLista.innerHTML = `<p class="ures-lista">Még nincs kiválasztott időpont.</p>`;
    return;
  }
...
if (kivalasztottLista) {
  ...
  kivalasztottLista.addEventListener(
    "click",
    (event) => {

      const gomb =
        event.target.closest(".kivalasztott-torles");

      if (!gomb) return;

      const index =
        Number(gomb.dataset.index);

      kivalasztottIdopontok.splice(
        index,
        1
      );
...
if (kuldesGomb) {
  ...
  kuldesGomb.addEventListener(
    ...
    async () => {
      ...
      if (kivalasztottIdopontok.length === 0) {
        alert("Először adj hozzá legalább egy időpontot.");
        return;
      }
